...
本文记录对若干公开中文 MUD 服务端/mudlib 源码的调研细节，并对照当前 Haskell 实现，提炼可迁移的玩法和架构设计。

综合结论与后续落地路线见 [MUD 调研综合结论与落地路线](./mud-design-synthesis.md)。本文保留为传统 mudlib 侧的证据和参考。
...
- `enable` 和 `prepare` 的显式服务端动作。
- prepared/enabled 存档。

还缺的不是动作入口，而是成本、时间和约束：

- busy / continue action，让学习、读书、自研、打坐不再是即时按钮。
- 精神或行动资源，区分潜能与一次行动成本。
- 更完整的师父教学上限、门派、师承和禁教规则。
- 吐纳、疗伤等内功长期资源动作。
- 客户端主动切换 enable/prepare 的 UI。
...
以下列表是早期调研得到的落地方向，当前已有一部分完成。新的统一优先级以 [MUD 调研综合结论与落地路线](./mud-design-synthesis.md) 为准。
...
2. 完善准备/切换武功。
   - server 已有 `enable` / `prepare`。
   - client 武学面板仍需提供切换。
   - 战斗普通攻击主要消费当前 prepared art。
